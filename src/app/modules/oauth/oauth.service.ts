...
class OAuthService {
  ...
  static getFacebookAuthUrl(): string {
    ...
    const scope = 'email,public_profile,pages_manage_posts,pages_read_engagement,instagram_basic,instagram_content_publish';
  ...
  static async exchangeFacebookCode(code: string): Promise<OAuthTokenResponse> {
    ...
    try {
      ...
      const responseError = error && typeof error === 'object' && 'response' in error ? 
        (error as { response?: { data?: { error?: { message?: string } } } }).response?.data?.error?.message : undefined;
      throw new AppError(
  ...
  static async getFacebookProfile(accessToken: string): Promise<SocialMediaProfile> {
    try {
  ...
  static async exchangeInstagramCode(code: string): Promise<OAuthTokenResponse> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const responseError = error && typeof error === 'object' && 'response' in error ? 
        (error as { response?: { data?: { error_message?: string } } }).response?.data?.error_message : undefined;
      throw new AppError(
        StatusCodes.BAD_REQUEST,
  ...
  static async getInstagramProfile(accessToken: string): Promise<SocialMediaProfile> {
    try {
  ...
  static async exchangeTwitterCode(code: string): Promise<OAuthTokenResponse> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const responseError = error && typeof error === 'object' && 'response' in error ? 
        (error as { response?: { data?: { error_description?: string } } }).response?.data?.error_description : undefined;
      throw new AppError(
        StatusCodes.BAD_REQUEST,
  ...
  static async getTwitterProfile(accessToken: string): Promise<SocialMediaProfile> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const responseError = error && typeof error === 'object' && 'response' in error ? 
        (error as { response?: { data?: { detail?: string } } }).response?.data?.detail : undefined;
      throw new AppError(
        StatusCodes.BAD_REQUEST,
  ...
  static async getLinkedInProfile(accessToken: string): Promise<SocialMediaProfile> {
    try {
